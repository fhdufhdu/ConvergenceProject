...
public class oracleTest {
                    ...
                    case 1: {
                        ...
                        switch (menu) {
                            ...
                            case 13: // 영화관 별 예매정보 등록
                            {
                                ...
                                ttDao.addTimeTable(
                                        insertInfoTimeTable(movieElem.getId(), screenElem.getId(), DTO.EMPTY_ID));
                            ...
                            case 14: // 영화관 별 예매정보 수정
                            {
                                ...
                                TimeTableDAO ttDao = new TimeTableDAO();
                                ArrayList<TimeTableDTO> ttlist = ttDao.getTimeTableList(
                                        insertInfoTimeTable(movieElem.getId(), screenElem.getId(), DTO.EMPTY_ID));
                                ...
                                ttDao.changeTimeTable(
                                        insertInfoTimeTable(movieElem.getId(), screenElem.getId(), ttElem.getId()));
                    ...
                    case 2: {
                        ...
                        switch (menu) {
                            ...
                            case 4: // 상영 시간표 조회
                            {
                                ...
                                ArrayList<TimeTableDTO> ttlist = ttDao.getTimeTableList(
                                        insertCurTimeTable("%", "%", DTO.EMPTY_ID), theaterElem.getId());
                            ...
                            case 9: // 평점 및 리뷰 수정
                            {
                                ...
                                rDao.changeReview(
                                        insertInfoReview(rvElem.getId(), rvElem.getMemberId(), rvElem.getMovieId()));
    ...
    public static void displayTheater(ArrayList<TheaterDTO> list) {
        for (int i = 0; i < list.size(); i++) {
            ...
            System.out.printf("[%d] %s | %s | %d | %d \n", i, temp.getName(), temp.getAddress(), temp.getTotalScreen(),
                    temp.getTotalSeats());
        }
    }
    ...
    public static void displayScreen(ArrayList<ScreenDTO> list) {
        for (int i = 0; i < list.size(); i++) {
            ...
            System.out.printf("[%d] %s | %s | %d | %d | %d \n", i, temp.getTheaterId(), temp.getName(),
                    temp.getTotalCapacity(), temp.getMaxRow(), temp.getMaxCol());
        }
    }
    ...
    public static void displayMovie(ArrayList<MovieDTO> list) {
        for (int i = 0; i < list.size(); i++) {
            ...
            System.out.printf("[%d] %s | %s | %s | %s | %s | %s | %s | %s | %s | %d \n", i, temp.getTitle(),
                    temp.getReleaseDate(), temp.getIsCurrent(), temp.getPlot(), temp.getPosterPath(),
                    temp.getStillCutPath(), temp.getTrailerPath(), temp.getDirector(), temp.getActor(), temp.getMin());
        }
    }
    ...
    public static void displayReview(ArrayList<ReviewDTO> rvList) {
        for (int i = 0; i < rvList.size(); i++) {
            ...
            System.out.printf("[%d] %s | %s | %d | %s  \n", i, temp.getMemberId(), temp.getMovieId(), temp.getStar(),
                    temp.getText());
        }
    }
