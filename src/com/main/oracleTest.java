...
public class oracleTest
{
                    ...
                    case 1:
                    {
                        ...
                        switch (menu)
                        {
                            ...
                            case 15: // 영화관 별 예매정보 제거
                            {
                                ...
                                ttDao.removeTimeTable(ttElem.getId());
    ...
    public static void displayMovie(ArrayList<MovieDTO> list)
    {
        for (int i = 0; i < list.size(); i++)
        {
            ...
            System.out.printf("[%d] %s | %s | %s | %s | %s | %s | %s | %s | %s | %d \n", i, temp.getTitle(), temp.getReleaseDate(), temp.getIsCurrent(), temp.getPlot(), temp.getPosterPath(), temp.getStillCutPath(), temp.getTrailerPath(), temp.getDirector(), temp.getActor(), temp.getMin());
        }
    }
    ...
    public static void displayTimeTable(ArrayList<TimeTableDTO> list) throws Exception
    {
        for (int i = 0; i < list.size(); i++)
        {
            ...
            System.out.printf("[%d] %s | %s | %s | %s | %s \n", i, temp.getMovieId(), temp.getScreenId(), temp.getStartTime(), temp.getEndTime(), temp.getType());
        }
    }
    ...
    public static void displayCurrentRsv(ArrayList<ReservationDTO> rsvList)
    {
        ...
        for (int i = 0; i < rsvList.size(); i++)
        {
            ...
            // System.out.printf("[%d] %s | %s | %d | %d | %d | %s \n", i, temp.getMemberId(), temp.getTimeTableId(), temp.getScreenRow(), temp.getScreenCol(), temp.getPrice(), temp.getCancel());
        }
    }
    
    /*
     * public static ReservationDTO insertInfoRsv(String member_id, String tt_id) { ReservationDTO empty = new ReservationDTO(); empty.setId("-1"); System.out.print("행을 선택해주세요 : "); empty.setScreenRow(sc.nextInt()); sc.nextLine(); System.out.print("열을 선택해주세요 : "); empty.setScreenCol(sc.nextInt()); sc.nextLine(); empty.setMemberId(member_id); empty.setTimeTableId(tt_id); empty.setCancel("0");
     * 
     * return empty; }
     */
