...
public class ScreenManage
{
	...
	void changeScreen(ActionEvent event) throws Exception
	{
		try
		{
			...
			mainGUI.writePacket(Protocol.PT_REQ_RENEWAL + "/" + Protocol.CS_REQ_SCREEN_CHANGE + "/" + theater.getId() + "/" + name + "/" + capacity + "/" + row + "/" + col);
			...
			while (true)
			{
				...
				if (packetCode.equals(Protocol.SC_RES_SCREEN_VIEW))
				{
					...
					switch (result)
					{
						case "1":
						{
							screen_list.add(new ScreenDTO(id, name, Integer.parseInt(capacity), Integer.parseInt(row), Integer.parseInt(col)));
							break;
						}
						case "2":
						{
